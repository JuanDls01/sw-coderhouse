...
export function SaberColorPicker() {
  const storedColor = useSyncExternalStore(subscribeToStorage, getStoredColor, () => DEFAULT_SABER_COLOR);
  const [selectedColor, setSelectedColor] = useState<SaberColor>(storedColor);

  useEffect(() => {
    document.documentElement.style.setProperty('--primary', SABER_COLOR_VAR[selectedColor]);
  }, [selectedColor]);
