...
export const swapiTools = {
  searchPeople: tool({
    ...
    toModelOutput: ({ output }) => {
      ...
      const { name, height, mass, hair_color, skin_color, eye_color, birth_year, gender } = output.data;
      return {
        type: 'text',
  ...
  searchPlanets: tool({
    ...
    toModelOutput: ({ output }) => {
      ...
      const { name, climate, terrain, population, diameter, gravity, surface_water } = output.data;
      return {
        type: 'text',
